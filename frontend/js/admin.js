...
// Only run this code if we're on admin.html page
if (window.location.pathname.includes('admin.html')) {
    ...
    async function loadProperties() {
        try {
            ...
            if (data.success) {
                ...
                if (totalPropertiesEl) totalPropertiesEl.innerText = totalCount;
    ...
    if (addPropertyForm) {
        addPropertyForm.addEventListener('submit', async (e) => {
            ...
            if (typeof showToast === 'function') showToast('Adding property... Please wait', 'info');
            ...
            try {
                ...
                if (data.success) {
                    if (typeof showToast === 'function') showToast('✅ Property added successfully!', 'success');
                    e.target.reset();
                    const galleryList = document.getElementById('addGalleryImagesList');
                    ...
                    if (typeof showToast === 'function') showToast(data.error || 'Failed to add property', 'error');
                }
            } catch (error) {
    ...
    window.editProperty = async (id) => {
        try {
            ...
            if (data.success) {
                ...
                const editGalleryList = document.getElementById('editGalleryImagesList');
                if (editGalleryList) editGalleryList.innerHTML = '';
                const editCustomList = document.getElementById('editCustomAmenitiesList');
                if (editCustomList) editCustomList.innerHTML = '';
    ...
    window.markGalleryImageForDelete = (index) => {
        if (!window.galleryImagesToDelete.includes(index)) {
            ...
            if (typeof showToast === 'function') showToast('Image marked for deletion', 'info');
        }
    };
    ...
    if (editPropertyForm) {
        editPropertyForm.addEventListener('submit', async (e) => {
            ...
            if (typeof showToast === 'function') showToast('Updating property...', 'info');
            ...
            try {
                ...
                const data = await response.json();
                if (data.success) {
                    if (typeof showToast === 'function') showToast('✅ Property updated successfully!', 'success');
                    window.galleryImagesToDelete = [];
                    closeModal();
    ...
    async function loadBookings() {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (pendingBookingsEl) pendingBookingsEl.innerText = pending;
    ...
    window.toggleAvailability = async (id, currentStatus) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (typeof showToast === 'function') showToast(data.error || 'Failed to toggle availability', 'error');
            }
        } catch (error) {
            if (typeof showToast === 'function') showToast(error.message, 'error');
        }
    ...
    window.deleteProperty = async (id) => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                if (typeof showToast === 'function') showToast('✅ Property deleted successfully!', 'success');
                setTimeout(() => loadProperties(), 1000);
            } else {
                if (typeof showToast === 'function') showToast(data.error || 'Failed to delete property', 'error');
            }
        } catch (error) {
            if (typeof showToast === 'function') showToast(error.message, 'error');
        }
    ...
    window.updateBookingStatus = async (id, status) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (typeof showToast === 'function') showToast(statusMessage, 'success');
                setTimeout(() => {
                    loadBookings();
                    loadAdminStats();
                }, 1000);
